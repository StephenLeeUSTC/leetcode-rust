...
impl Solution {
    pub fn longest_common_subsequence(text1: String, text2: String) -> i32 {
        ...
        for (i, c) in text1.chars().enumerate() {
            for (j, cc) in text2.chars().enumerate() {
                if c == cc {
                    dp[i + 1][j + 1] = dp[i][j] + 1;
                } else {
...
mod tests {
    ...
    fn test_1143() {
        ...
        let text3 = "abc".to_string();
        let text4 = "abc".to_string();
        assert_eq!(Solution::longest_common_subsequence(text3, text4), 3);

        let text5 = "abc".to_string();
        let text6 = "def".to_string();
        assert_eq!(Solution::longest_common_subsequence(text5, text6), 0);
    }
}
